...
public class MontbellOrderDemon {
	...
	public void execute(WebDriver driver) {
		...
		try {
			...
			e.printStackTrace();
			NieUtil.log(logFile, "[ERROR][Montbell][scanTBOrder]" + e.getMessage());
			NieUtil.log(logFile, e);
		}

	}

	private void scanTBOrder(WebDriver driver) throws UnsupportedOperationException, IOException {
		...
		for(OrderInfoObject order:orders){
			...
			for(OrderInfoObject orderMerge:ordersMerge){
				if(order.orderObject.buyerName.equals(orderMerge.orderObject.buyerName)){
					existFlag = true;
					merge = orderMerge;
					break;
				}
		...
		for(OrderInfoObject order:ordersMerge){
			...
			for (OrderDetailObject dtl : order.orderDtlList) {
				...
				try {
					TaobaoOrderProductInfo info = MontBellUtil.readTaobaoProductInfo(dtl.sku);
					productList += info.productId + "," + info.colorName + "," + info.sizeName + ";";
				} catch (Exception e) {
			...
			NieUtil.log(logFile, "[INFO][Service:saveTBOrder][Param]" + "[maijia]" + maijia + "[dingdanhao]" + dingdanhao);

			String rslt = NieUtil.httpGet(NieConfig.getConfig("montbell.order.service.url"), param);

			if (!StringUtil.isBlank(rslt)) {
				NieUtil.log(logFile, "[INFO][Service:saveTBOrder][RESULT]" + rslt);
				continue;
			}
		}
	}
